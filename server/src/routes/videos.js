...
router.get('/videos/:name/proxy', requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    // 'needed' means the probe says the browser cannot show this clip; 'none'
    // means it is already fine. Only the former starts an encode.
    const state = current.state === 'needed' ? await ensureProxy(name) : current;
    res.json(state);
  } catch (err) {
    next(err);
  }
